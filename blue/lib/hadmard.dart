...
class Draging extends StatefulWidget {
  final String data;
  const Draging({super.key, required this.data});

  @override
  _DragingState createState() => _DragingState();
}

class _DragingState extends State<Draging> {
  ...
  void addGate(String gate, int qubit, int position) {
    setState(() {
      if (position < circuit[qubit].length) {
        if (gate == 'H') {
          // Find adjacent qubit for Hadamard control/target
          for (int i = 0; i < circuit.length; i++) {
            if (i != qubit && circuit[i][position].isEmpty) {
              if (i == (qubit + 1) % circuit.length ||
                  i == (qubit - 1 + circuit.length) % circuit.length) {
                circuit[qubit][position] = 'H_control';
                circuit[i][position] = 'H_target';
                break;
              }
            }
          }
