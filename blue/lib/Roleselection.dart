...
import 'package:blue/dragdrop1.2.dart';
import 'package:blue/dragdrop1.dart';
import 'package:flutter/material.dart';

class SelectionScreen extends StatefulWidget {
  final String data;
  const SelectionScreen({super.key, required this.data});
  @override
  ...
  void _validateRoles() {
    ...
    if (_selectedRole == _receivedRole) {
      ...
      if (_selectedRole == "Alice") {
        Navigator.push(
          context,
          MaterialPageRoute(builder: (context) => const QuantumDesignHome()),
        );
      } else if (_selectedRole == "Bob") {
        Navigator.push(
          context,
          MaterialPageRoute(
            builder: (context) => QuantumCircuitDesignerHome(
              getdata: widget.data,
            ),
          ),
        );
