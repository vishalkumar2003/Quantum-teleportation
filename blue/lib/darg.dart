...
class Drag2Home extends StatefulWidget {
  final String initialQubit;
  final String getdata;
  const Drag2Home(
      {super.key, required this.initialQubit, required this.getdata});

  @override
  _Drag2HomeState createState() => _Drag2HomeState();
}

class _Drag2HomeState extends State<Drag2Home> {
            ...
            child: InteractiveViewer(
              ...
              child: Padding(
                ...
                child: Container(
                  ...
                  child: CustomPaint(
                    painter:
                        Drag2CircuitPainter(circuit: circuit, scale: scale),
                    child: Stack(
                      children: [
...
class Drag2GateItem extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Draggable<String>(
      data: gateName,
      feedback: Drag2GateItemWidget(
          gateName: gateName, gateSymbol: gateSymbol, color: color),
      childWhenDragging: Drag2GateItemWidget(
          gateName: gateName,
          gateSymbol: gateSymbol,
          color: color.withOpacity(0.5)),
      child: Drag2GateItemWidget(
          gateName: gateName, gateSymbol: gateSymbol, color: color),
    );
  }
}

class Drag2GateItemWidget extends StatelessWidget {
  final String gateName;
  final String gateSymbol;
  final Color color;

  const Drag2GateItemWidget({
    Key? key,
    required this.gateName,
    required this.gateSymbol,
    required this.color,
...
class Drag2CircuitPainter extends CustomPainter {
  final List<List<String>> circuit;
  final double scale;

  Drag2CircuitPainter({required this.circuit, this.scale = 1.0});
